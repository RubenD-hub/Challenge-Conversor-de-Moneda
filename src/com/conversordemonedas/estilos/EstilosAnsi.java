package com.conversordemonedas.estilos;

public class EstilosAnsi {
    // Reset
    public static final String RESET = "\u001B[0m";

    // Estilos
    public static final String Negrita = "\u001B[1m";
    public static final String Cursiva = "\u001B[3m";
    public static final String Subrayado = "\u001B[4m";
    public static final String Inverso = "\u001B[7m";

    // Colores de texto
    public static final String NEGRO = "\u001B[30m";
    public static final String ROJO = "\u001B[31m";
    public static final String VERDE = "\u001B[32m";
    public static final String AMARILLO = "\u001B[33m";
    public static final String AZUL = "\u001B[34m";
    public static final String MAGENTA = "\u001B[35m";
    public static final String CYAN = "\u001B[36m";
    public static final String BLANCO = "\u001B[37m";

    // Fondos
    public static final String FONDO_NEGRO = "\u001B[40m";
    public static final String FONDO_ROJO = "\u001B[41m";
    public static final String FONDO_VERDE = "\u001B[42m";
    public static final String FONDO_AMARILLO = "\u001B[43m";
    public static final String FONDO_AZUL = "\u001B[44m";
    public static final String FONDO_MAGENTA = "\u001B[45m";
    public static final String FONDO_CYAN = "\u001B[46m";
    public static final String FONDO_BLANCO = "\u001B[47m";

    // Utilidad para aplicar color más fácilmente
    public static boolean COLORES_ACTIVADOS = true;

    public static String aplicar(String texto, String... estilos) {
        if (!COLORES_ACTIVADOS) return texto;
        StringBuilder sb = new StringBuilder();
        for (String estilo : estilos) {
            sb.append(estilo);
        }
        sb.append(texto).append(RESET);
        return sb.toString();
    }
}
